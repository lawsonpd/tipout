from tipout.models import Employee, Paycheck, Tip, Expenditure, Expense
...
def balancer(budgets, expenditures):
...
def budget_for_specific_day(emp, date):
    ...
    day_expense_cost = sum([ exp.cost for exp in expenses ]) / 30

    # expenditures for the day
    expenditures_for_day_query = Expenditure.objects.filter(owner=emp, date=date)
    expenditures_for_day = sum([ exp.cost for exp in expenditures_today_query ])
    ...
    return tips_for_day + daily_avg_from_paycheck(paycheck_amts) - daily_expense_cost - expenditures_today
